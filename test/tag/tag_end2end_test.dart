...
void main() {
  group('end2end tests', () {
    test('minimal', () async {
      ...
      _expectTagging(
        tagger.runtimeTags,
        tags: {'runtime:native-jit', 'runtime:native-aot', 'runtime:web'},
        explanations: isEmpty,
      );
...
'''),
      ...
      _expectTagging(tagger.runtimeTags, tags: {'runtime:web'});
    });
...
'''),
        ...
        packageWithPathDeps(
          'my_dependency',
          lib: [d.file('my_dependency.dart', 'int fourtyThree() => 43;')],
          pubspecExtras: {
            'environment': {'flutter': '>=1.2.0<=2.0.0'},
            'flutter': {
              'plugin': {
    ...
    test('Declaring top-level platforms', () async {
      final descriptor = d.dir('cache', [
        packageWithPathDeps(
          'my_package',
          pubspecExtras: {
            'platforms': {'windows': null, 'android': null},
          },
        ),
      ]);
      await descriptor.create();
...
'''),
      ...
      _expectTagging(
        ...
        explanations: contains(
          _explanation(finding: 'Package not compatible with platform Android'),
        ),
      );
      _expectTagging(
        tagger.runtimeTags,
        tags: {'runtime:native-jit'},
        explanations: {
          _explanation(
...
'''),
    ...
    test(
      'Excluded to invalid prefixed @JS annotation on extension type constructor',
      () async {
        final descriptor = d.dir('cache', [
          packageWithPathDeps(
            'my_package_prefixed',
...
extension type ZXingWasmReaderOptions._(js.JSObject _) implements js.JSObject {
  @js.JS('ZXingWasmReaderOptions')
  external factory ZXingWasmReaderOptions.withFormats();
}
'''),
            ],
        ...
        await descriptor.create();
        final tagger = Tagger('${descriptor.io.path}/my_package_prefixed');
        _expectTagging(
          tagger.wasmReadyTag,
          tags: isNot(contains('is:wasm-ready')),
          explanations: contains(
