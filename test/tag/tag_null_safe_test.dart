...
void expectTagging(
  ...
  Object? tags = anything,
  Object? explanations = anything,
}) {
...
void main() {
  group('null-safe', () {
    test(
      'compliant package depending on compliant package gets the tag',
      () async {
    ...
    test(
      'opting a library not reachable from public out still gets tag',
      () async {
        final descriptor = d.dir('cache', [
          packageWithPathDeps(
            'my_package',
...
// @dart = 2.3'''),
    ...
    test(
      'opting a library to older version still allowing null safety is ok',
      () async {
        final descriptor = d.dir('cache', [
          packageWithPathDeps(
            'my_package',
            ...
            lib: [
              d.file('my_package.dart', 'int fourtyTwo() => 42;'),
              d.dir('src', [d.file('stray_file.dart', '// @dart = 2.12\n')]),
            ],
          ),
        ]);
        await descriptor.create();
    ...
    test('allow non-imported opt-outed library in dependency', () async {
      final descriptor = d.dir('cache', [
        ...
        packageWithPathDeps(
          'my_dependency',
          sdkConstraint: '>=2.12.0 <3.0.0',
          lib: [d.file('my_dependency.dart', '// @dart = 2.9')],
        ),
      ]);
      await descriptor.create();
      final tagger = Tagger('${descriptor.io.path}/my_package');
...
'''),
    ...
    test('Broken imports gets reported', () async {
      ...
      final tagDetectionFailed = contains(
        explanation(finding: 'Tag detection failed.'),
      );

      expectTagging(tagger.nullSafetyTags, explanations: tagDetectionFailed);
      expectTagging(tagger.platformTags, explanations: tagDetectionFailed);
      expectTagging(tagger.sdkTags, explanations: tagDetectionFailed);
      expectTagging(tagger.runtimeTags, explanations: tagDetectionFailed);
    });
    ...
    test('dart-ext: import gets ignored', () async {
      final descriptor = d.dir('cache', [
        packageWithPathDeps(
          'my_package',
          sdkConstraint: '>=2.12.0 <3.0.0',
          lib: [d.file('my_package.dart', 'import "dart-ext:rpi_gpio_ext";')],
        ),
      ]);
      await descriptor.create();
      final tagger = Tagger(p.join(descriptor.io.path, 'my_package'));
...
Matcher explanation({
  ...
  return allOf(HasFinding(finding), HasDescription(explanation));
}
