...
Future<Maintenance> detectMaintenance(
  ...
  if (changelogFile == null) {
    maintenanceSuggestions.add(Suggestion.warning(
        ...
        score: 20.0));
  } else if (changelogFile.path.toLowerCase().endsWith('.md')) {
    final suggestion = await analyzeMarkdownFile(changelogFile, pkgDir: pkgDir);
    if (suggestion != null) {
  ...
  if (readmeFile == null) {
    maintenanceSuggestions.add(Suggestion.warning(
        ...
        score: 30.0));
  } else if (readmeFile.path.toLowerCase().endsWith('.md')) {
  ...
  } else if (exampleFile.path.toLowerCase().endsWith('.md')) {
