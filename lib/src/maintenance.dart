...
Future<Maintenance> detectMaintenance(
  ...
  if (readmeFile == null) {
    ...
    readmeContent = await readmeFile.readAsString();
  ...
  final readmeHasExample = _hasExampleInReadmeMarkdown(readmeContent);
  if (exampleFile == null && !readmeHasExample) {
...
bool _hasExampleInReadmeMarkdown(String text) {
  if (text == null || text.isEmpty) return false;
  final lines = text.split('\n').where((l) => l.trim().isNotEmpty).toList();
  final sectionHeaders = List.generate(lines.length, (i) => i)
      .where((i) => lines[i].startsWith('#'))
      .toList();
  for (var i = 0; i < sectionHeaders.length; i++) {
    final headerIndex = sectionHeaders[i];
    final lineText = lines[headerIndex].split('#').last.trim().toLowerCase();
    if (lineText.contains('example') || lineText.contains('usage')) {
      if (i == sectionHeaders.length - 1 && headerIndex < lines.length - 2) {
        return true;
      }
      if (i < sectionHeaders.length - 1 &&
          sectionHeaders[i + 1] - headerIndex >= 2) {
        return true;
      }
    }
  }
